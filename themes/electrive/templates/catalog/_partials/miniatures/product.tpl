...
{block name='product_miniature_item'}
  <article class="product-miniature js-product-miniature col-xs-12 propadding" data-id-product="{$product.id_product}"
    ...
    <div class="thumbnail-container text-xs-center">
        ...
        {block name='product_thumbnail'}
          {if $product.cover}
            <a href="{$product.url}" class="thumbnail product-thumbnail first-img">
              ...
              {foreach from=$product.images item=image}
                {if $count==0}
                  <img class="second-img img-responsive center-block" src="{$image.bySize.home_default.url}" alt="{$image.legend}"
                    title="{$image.legend}" itemprop="image" width="{$product.cover.bySize.home_default.width}"
                    height="{$product.cover.bySize.home_default.height}">
                {/if} {$count=$count+1}
              {/foreach}
            </a>
          {else}
            <a href="{$product.url}" class="thumbnail product-thumbnail">
              <img class="img-responsive center-block" src="{$urls.no_picture_image.bySize.home_default.url}">
            </a>
          {/if}
      ...
      <div itemprop="offers" itemtype="http://schema.org/Offer" itemscope>
        ...
        <meta itemprop="description"
          content="{if $product.description_short}{$product.description_short}{else}{$product.name}{/if}">
      ...
      <div class="wb-product-desc product-description text-xs-center">
        ...
        {block name='product_description_short'}
          <div id="product-description-short-{$product.id}" itemprop="description" class="listds">
        ...
        <div class="button-group allpbt">
          ...
          <div class="absbtn">
            ...
            {block name='quick_view'}
              <a class="quick-view quick" title="{l s='Quick-view' d='Shop.Theme.Catalog'}" href="#"
                data-link-action="quickview">
                <svg width="18px" height="18px">
                  <use xlink:href="#bquick" />
                </svg><span>{l s='Quick-view' d='Shop.Theme.Catalog'}</span>
              </a>
            {/block}
            <button title="{l s='Wishlist' d='Shop.Theme.Catalog'}" class="wish" id="wishlist_button"
              onclick="WishlistCart('wishlist_block_list', 'add', '{$product.id_product|intval}', {$product.id_product_attribute}, 1); return false;">
              <svg width="20px" height="20px">
                <use xlink:href="#heart"></use>
              </svg>
              <span>{l s='Wishlist' d='Shop.Theme.Catalog'}</span>
            </button>
            <div class="add-cart lg-add-to-cart">
              <form action="{$urls.pages.cart}" method="post" class="add-to-cart-or-refresh">
                <input type="hidden" name="token" value="{$static_token}">
                <input type="hidden" name="id_product" value="{$product.id}" class="product_page_product_id">
                <input type="hidden" name="qty" value="1">
                {if isset($product) && ($product.quantity < 1) }
                  <button data-toggle="tooltip" title="{l s='stock out' d='Shop.Theme.Catalog'}"
                    data-button-action="add-to-cart" class="cartb" disabled>
                    <svg width="18px" height="18px">
                      <use xlink:href="#pcart"></use>
                    </svg><span> {l s='stock out' d='Shop.Theme.Catalog'}</span>
                  </button>
                {else}
                  <button data-toggle="tooltip" title="{l s='add to cart' d='Shop.Theme.Catalog'}"
                    data-button-action="add-to-cart" class="cartb">
                    <svg width="68" height="44">
                      <use xlink:href="#pcart"></use>
                    </svg><span> + {l s='add to cart' d='Shop.Theme.Catalog'}</span>
                  </button>
                {/if}
              </form>
            </div>
