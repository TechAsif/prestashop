                ...
                <table id="mp_b" class="table table-striped" >
					...
					{foreach $product_lists as $key => $product}
					   <a href="#">
                        ...
                        <tr >
						   <a href="#">
                            <td data-toggle="collapse" data-target="#{$product.id_mp_product}" class="accordion-toggle">{$product.id_mp_product}</td>
                            <td data-toggle="collapse" data-target="#{$product.id_mp_product}" class="accordion-toggle">{$product.name}</td>
							{if $product.unity eq '' }
                            <td data-toggle="collapse" data-target="#{$product.id_mp_product}" class="accordion-toggle">{$product.from_quantity} Number+</td>
                            
							{else}
                            <td data-toggle="collapse" data-target="#{$product.id_mp_product}" class="accordion-toggle">{$product.from_quantity} {$product.unity}+</td>
							{/if}

                            <td data-toggle="collapse" data-target="#{$product.id_mp_product}" class="accordion-toggle">{{$product.price} -{$product.reduction}}</td>
							</a>
                     ...
                     <td><a title="{l s='Duplicate' mod='marketplace'}" href="{$link->getModuleLink('marketplace', 'youroffersupdateproduct', ['id_mp_product' => $product.id_mp_product, 'duplicateproduct' => 1])}" class="duplicate_mp_product">
						<button class="btn btn-primary btn-sm wk_edit_profile_btn">
							{l s='Sell Like this' mod='marketplace'}
						</button>
                        </td>
                        ...
                        <tr>
                            <td colspan="12" class="hiddenRow">
                                <div class="accordian-body collapse hiddenRow" id="{$product.id_mp_product}">
                                    <table class="table table-striped">
									    ...
									    {foreach $specificAllProduct as $key => $specificProduct}	
										    {if $product.id_mp_product eq $specificProduct.id_mp_product }	
												<tr>
													...
													{* <td>demo</td> *}
													<td>{{$specificProduct.price} - {$specificProduct.reduction}}</td>
