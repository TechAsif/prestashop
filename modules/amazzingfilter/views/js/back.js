	...
	$.extend(af, {
		...
		instantSettings: {
			...
			autosaveIfRequired($el) {
				if ($el.hasClass('no-autosaving-once')) {
					...
					if ($el.attr('name').startsWith('merged_')) {
						af.merged.toggleMenuItem($el.attr('name').split('merged_')[1].slice(0, -1));
					}
				}
			}
		},
		oos: {
			...
			bindEvents: function() {
				...
				$('.oos-option.combinations-stock').find('input').on('change', function() {
