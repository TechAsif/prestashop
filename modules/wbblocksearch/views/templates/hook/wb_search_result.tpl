...
<div class="content-ajax-search">
  <div class="search-title text-xs-left">
    {l s='The result search for ' mod='wbblocksearch'} "{$query|escape:'quotes':'UTF-8'}"
    <span>({$search_Total|escape:'quotes':'UTF-8'} {l s=' results have been found.' mod='wbblocksearch'})</span>
      ...
      {foreach from=$searchResults item=product key=key}
        ...
        <li class="item">
          <div class="image float-xs-left">
            <a href="{$product.link|escape:'quotes':'UTF-8'}" class="product_img_link"
              title="{$product.name|escape:'htmlall':'UTF-8'}">
              <img
                src="{$link->getImageLink($product.link_rewrite|escape:'quotes':'UTF-8',$product.id_image ,'home_default')}"
          ...
          <div class="wbContent">
            <h5 class="product-name">
              <a class="product-name" href="{$product.link|escape:'quotes':'UTF-8'}" class="product_img_link"
                title="{$product.name|escape:'htmlall':'UTF-8'}">
            ...
            <div class="content_price product-price-and-shipping">
              {if isset($product.show_price) && $product.show_price && !isset($restricted_country_mode)}
                <span itemprop="price"
                  class="price {if isset($product.specific_prices.reduction) && $product.specific_prices.reduction > 0} special-price{/if}">
  ...
  {if $search_Total > $limit_item}
    <p class="title_showall_text">{l s='Search limited to ' mod='wbblocksearch'} {$limit_item|escape:'quotes':'UTF-8'}
      {l s='products' mod='wbblocksearch'}</p>
