...
{if ! $order_list}
  <p class="warning warn">{l s='There are no orders that use any of dhl carriers in your selection' mod='paperfly'}</p>
{else}
...
<div class="row dhl_panel">
  ...
  <div id="ordercarrier-{$order_line.id_order_carrier|escape:'html':'UTF-8'}" class="panel"{if isset($error_order_line) && in_array($order_line['id_order'], $error_order_line)} style="border-color: red;"{elseif isset($warning_order_line) && in_array($order_line['id_order'], $warning_order_line)} style="border-color: orange;"{elseif isset($success_order_line) && in_array($order_line['id_order'], $success_order_line)} style="border-color: #00ff00;"{else}{/if}>
    <div class="panel-heading">{l s='Order #' mod='paperfly'} <span class="badge">{$order_line.id_order|escape:'html':'UTF-8'}</span>
        {l s='Order Ref.' mod='paperfly'} <span class="badge">{$order_line.reference|escape:'html':'UTF-8'}</span>
		{l s='Customer' mod='paperfly'} <span class="badge">{$order_line.customer|escape:'html':'UTF-8'}</span>
		{l s='Country' mod='paperfly'} <span class="badge">{$order_line.country|escape:'html':'UTF-8'}</span>
        <a class="button" href="?tab=AdminOrders&amp;id_order={$order_line['id_order']|intval}&amp;vieworder&amp;token={getAdminToken tab='AdminOrders'}">
            {l s='View order' mod='paperfly'}
        </a>
	</div>
    ...
    <table class="table std" cellspacing="0" cellpadding="0" style="width:100%;">
		<thead>
			  <tr>
				...
				<th>{l s='Options' mod='paperfly'}</th>
				<th>{l s='Action' mod='paperfly'}</th>
			  </tr>
		  </thead>
        {if isset($order_line.labels)}
          <tbody>
            {if count($order_line.labels) > 1}<tr><td colspan="6"><a href="#" id="toggleAllDHLLabelsForOrder">{l s='Show/hide all labels for this order' mod='paperfly'}</a></td></tr>{/if}
            {foreach from=$order_line.labels item=carrier name=dhl_label}
              <tr{if !$smarty.foreach.dhl_label.last} class="hiddenLabel" style="display: none;"{/if}>
                ...
                <td>
                    <label>{l s='Weight' mod='paperfly'}</label>: {Tools::ps_round($carrier.packages[0]['weight'], 1)|escape:'htmlall':'UTF-8'} {l s='kg' mod='paperfly'};
                    <label>{l s='Length' mod='paperfly'}</label>: {$carrier.packages[0]['length']|escape:'htmlall':'UTF-8'} {l s='cm' mod='paperfly'} x
                    <label>{l s='Width' mod='paperfly'}</label>: {$carrier.packages[0]['width']|escape:'htmlall':'UTF-8'} {l s='cm' mod='paperfly'} x
                    <label>{l s='Height' mod='paperfly'}</label>: {$carrier.packages[0]['height']|escape:'htmlall':'UTF-8'} {l s='cm' mod='paperfly'};
                </td>
                  <td>
                      <div id="dhl_options">
                          ...
                          <div>
                              <label>{l s='Minimum age' mod='paperfly'}</label>
                              <div class="input-group fixed-width-x2 pull-right">
                                  {$carrier.options_decoded['CheckMinimumAge']['MinimumAge']|escape:'htmlall':'UTF-8'} {l s='years' mod='paperfly'}
                              </div>
                          </div>
                          {/if}
                      </div>
                ...
                <td>
                    <label>{l s='Shipment number' mod='paperfly'}</label>: {$carrier.shipment_number|escape:'htmlall':'UTF-8'}<br>
                    {if $carrier.label_url}
                        ...
                        <a href="{$carrier.label_url|escape:'html':'UTF-8'}" class="btn btn-primary" target="_blank"><i class="icon-print"></i> {l s='Print label' mod='paperfly'}</a>
                    {/if}
                    {if $carrier.return_label_url != ''}
                        <a class="btn btn-primary" href="{$carrier.return_label_url|escape:'html':'UTF-8'}" target="_blank"><i class="icon-print"></i> {l s='Print return label' mod='paperfly'}</a>
                    {/if}
                    {if $carrier.export_label_url != ''}
                        <a class="btn btn-primary" href="{$carrier.export_label_url|escape:'html':'UTF-8'}" target="_blank"><i class="icon-print"></i> {l s='Print export doc' mod='paperfly'}</a>
                    {/if}
                    {if $carrier.cod_label_url != ''}
                        <a class="btn btn-primary" href="{$carrier.cod_label_url|escape:'html':'UTF-8'}" target="_blank"><i class="icon-print"></i> {l s='Print COD doc' mod='paperfly'}</a>
                    {/if}
                    {if $carrier.tracking_url}
                    <a class="btn btn-default" href="{$carrier.tracking_url|escape:'html':'UTF-8'}" target="_blank"><i class="icon-search"></i> {l s='Tracking' mod='paperfly'}</a>
                    {/if}
                </td>
            </tr>
         {/foreach}
        ...
        <tr>
            ...
            <td align="left" colspan="3">
                <div id="dhl_params" class="row">
                    <div class="col-lg-3">
                        <label for="width">{l s='Weight' mod='paperfly'}</label>
                        <div class="input-group">
                            ...
                            <span class="input-group-addon">{l s='kg' mod='paperfly'}</span>
                        </div>
                    </div>
                    <div class="col-lg-3">
                        <label for="width">{l s='Width' mod='paperfly'}</label>
			            <div class="input-group">
                            ...
                            <span class="input-group-addon">{l s='cm' mod='paperfly'}</span>
					   </div>
                    </div>
                    <div class="col-lg-3">
                        <label for="height">{l s='Height' mod='paperfly'}</label>
                        <div class="input-group">
                    ...
                    <div class="col-lg-3">
                        <label for="depth">{l s='Depth' mod='paperfly'}</label>
                        <div class="input-group">
                ...
                <div class="row param-footer">
                    <div class="col-lg-12">
                        <button type="button" class="button btn btn-default pull-right" id="submitDhlAdditServices" name="submitDhlAdditServices"><i class="icon-angle-double-down"></i> {l s='Additional services' mod='paperfly'} </button>
                        <button  type="button" class="button btn btn-default" id="submitDhlUpdateAddress" name="submitDhlUpdateAddress"><i class="icon-angle-double-down"></i> {l s='Update delivery address' mod='paperfly'} </button>
                        <button  type="button" class="button btn btn-default" id="submitDhlExportDocuments" name="submitDhlExportDocuments"><i class="icon-angle-double-down"></i> {l s='Export documents' mod='paperfly'} </button>
                    </div>
                </div>
            </td>
        </tr>
    ...
    {else}
        <span class="alert-warning">{l s='no flingex carrier' mod='paperfly'}</span>
    {/if}
	</div>
  {/foreach}
  <div class="panel">
	  <p class="alert alert-info">{l s='The labels will be generated only for those orders that don\'t have them generated already, the existing labels will not change' mod='paperfly'}</p>
	  <p>
		<button type="submit" class="btn btn-success" name="generateMultipleLabels" id="generateMultipleLabels">{l s='Generate labels' mod='paperfly'}</button>
		<button type="submit" class="btn btn-primary" name="printMultipleLabels" id="printMultipleLabels"><i class="icon-print"></i> {l s='Print last labels' mod='paperfly'}</button>
	  </p>
  </div>
  </form>
  </div>
