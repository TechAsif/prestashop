...
//        );
    ...
    public function createDhlDeliveryLabel(
        ...
        $shipper = $this->flingex_api->getShipper($id_shop);

        $def = $this->flingex_api->getDefinedProducts($aproduct_code[0], $receiver['countryISOCode'], $shipper['countryISOCode'], $this->flingex_api->getApiVersion());

        if (self::getConfig('DHL_MODE', $id_shop) == 1) {
            $ekp = self::getConfig('DHL_LIVE_EKP', $id_shop);
        } else {
            $ekp = FlingexDHLFlingexApi::$dhl_sbx_ekp[$this->flingex_api->getApiVersion()];
        }
    ...
    public function getLabelData($id_order_carrier)
    {
        ...
        if (count($id_order_carrier) > 0) {
            ...
            foreach ($selected_values as $selected_value_index => $selected_value) {
                ...
                $selected_values[$selected_value_index]['packages'] = FLINGEXLabel::getPackages(
                    $selected_value['id_flingex_label']
                );
                ...
                $selected_values[$selected_value_index]['tracking_url'] = str_replace(
                    ...
                    FlingexDHLFlingexApi::$tracking_url
                );
            }
    ...
    public function displayDHLAdminOrder($params)
    {
        ...
        if (Tools::getIsset('submitDHLDPDhlLabelRequest') || Tools::getIsset('submitDHLDPDhlLabelWithReturnRequest') || Tools::getIsset('submitDHLDPDhlLabelReturnRequest')) {
            ...
            if (is_array($formatted_products[0])) {
                $aproduct_code = explode(':', $product_code);
                $product_def = $this->flingex_api->getDefinedProducts(
                    $aproduct_code[0],
                    isset($receiver_address['Address']['Origin']['countryISOCode'])?$receiver_address['Address']['Origin']['countryISOCode']:'DE',
                    $this->flingex_api->getShipperCountry($order->id_shop),
                    $this->flingex_api->getApiVersion()
                );
            ...
            foreach ($shipping as $shipping_item) {
                ...
                $this->context->smarty->assign(
                    array(
                        ...
                        'flingex_dhl_products_params' => $dhl_products,
        ...
        if (($this->context->controller->controller_name == 'AdminOrders' || $this->context->controller instanceof AdminOrdersController) && !$id_order) {
            ...
            if (Tools::getValue('view') == 'settings_dp') {
                ...
                if (Tools::getValue('view') == 'generateLabels') {
                    ...
                    $dhl_products = $this->flingex_api->getDefinedProducts('', '', self::getConfig('DHL_COUNTRY'), self::getConfig('DHL_API_VERSION'));
                    ...
                    $dhl_gogreen_options_js[] = $dhl_gogreen_option_js;
                    $script .= '<script>
                    var defined_dhl_api_versions = ' . Tools::jsonEncode(FlingexDHLFlingexApi::$supported_shipper_countries) . ';
...
//                'information' => array('name' => $this->l('Information'), 'icon' => ''),
    ...
    protected function displayFormDHLSettings()
    {
        ...
        $this->context->smarty->assign(
            array(
                ...
                'flingex_dhl_api_version' => Tools::getValue('DHLDP_DHL_API_VERSION', self::getConfig('DHL_API_VERSION')?self::getConfig('DHL_API_VERSION'):'2.1'),
            )
        );
    ...
    protected function getFormattedAddedDhlProducts($added_dhl_products, $to_country = '', $from_country = '', $api_version = '')
    {
        ...
        if (is_array($added_dhl_products)) {
            $dhl_products = $this->flingex_api->getDefinedProducts('', $to_country, $from_country, $api_version);
                    ...
                    'form' => array(
                        ...
                        'input'       => array(
                            ...
                            array(
                                ...
                                'type'     => 'select',
                                'label'    => $this->l('FLINGEX API version'),
                                'class'    => 't',
                                'required' => true,
    ...
    protected function getFormFieldsDHLSettings()
    {
            ...
            array(
                ...
                'form2' => array(
                    'form' => array(
                        ...
                        'legend'      => array(
                            'title' => $this->l('FLINGEX products'),
                            'icon'  => 'icon-circle',
                        ),
                        'description' => $this->l('Please enter products of FLINGEX and participation numbers in according your FLINGEX account. You can identify FLINGEX products and participation numbers on "http://www.intraship.de". Sign up and navigate to "Versandabwicklung Paket" -> "Neuer Auftrag". You will find names of products and participation numbers which located at the end of each selection option in selection field "Standart produkt". Participation numbers should be exactly 2  characters long, for example, "01", "02".'),
                        'input'       => array(
                            array(
                                'name'  => 'dhl_products',
                                'type'  => 'free',
                                'label' => $this->l('FLINGEX Products'),
                                'desc'  => $this->l(''),
                                'class' => 't',
                            ),
                            array(
                    ...
                    'form' => array(
                        ...
                        'input'  => array(
                            ...
                            array(
                                'type'     => (_PS_VERSION_ < '1.6.0.0') ? 'radio' : 'switch',
                                'name'     => 'DHLDP_DHL_PFPS',
                                'label'    => $this->l('Enable FLINGEX Postfiliales and FLINGEX Packstations'),
                            ...
                            array(
                                ...
                                'desc'     => $this->l('Google API key is used for showing map with locations of FLINGEX Postfiliales and FLINGEX Packstations. It is required if you set enabled FLINGEX Postfiliales and FLINGEX Packstations.'),
                            ),
                            array(
                                'type'     => (_PS_VERSION_ < '1.6.0.0') ? 'radio' : 'switch',
                            ...
                            array(
                                ...
                                'desc'    => $this->l('Order status will be changed "Shipped" automatically after creating FLINGEX label'),
                                'options' => array(
                                    'query' => array_merge(
                                        array(
                                            array(
                            ...
                            array(
                                ...
                                'label'    => $this->l('Enable customer confirmation for permission transferring private information to FLINGEX service'),
                                'desc'    => $this->l('If you are enable it, then shop will ask customer permission for sending e-mail address and phone number to FLINGEX service in frontend. If you disabled it, then e-mail address and phone number will be sent to FLINGEX service by default.'),
                                'class'    => (_PS_VERSION_ < '1.6.0.0') ? 't' : '',
                                'is_bool'  => true,
                                'disabled' => false,
                                'values'   => array(
        ...
        $form_fields['form4'] = array(
            'form' => array(
                'legend' => array(
                    'title' => $this->l('FLINGEX Retoure settings and additional settings for Merchandise return (RMA)'),
                    'icon' => 'icon-truck'
                ),
                ...
                'input' => array(
                    ...
                    array(
                        ...
                        'form_group_class' => 'hide flingex_dhl_ra',
                    ),
                    array(
                        'type' => 'select',
        ...
        $form_fields['form6'] = array(
            'form' => array(
                ...
                'description' => $this->l('Bank data can be provided here for different purposes. E.g. if COD is booked as service, bank data must be provided by FLINGEX customer (mandatory server logic). The collected money will be transferred to specified bank account.'),
                'input'       => array(
                    array(
        ...
        /*****create setup from for flingex settings******/
        $form_fields['form7'] = array(
            'form' => array(
                'id_form'     => 'flingex_setup',
                'legend'      => array(
                    'title' => $this->l('Paper Fly Account Setup'),
                    'icon'  => 'icon-circle',
                ),
                'description' => $this->l('Provide flingex credential to connect'),
                'input'       => array(
                    array(
                        'name'     => 'SANDBOX_URL',
                        'type'     => 'text',
                    ...
                    array(
                        'name'     => 'FLINGEX_KEY',
                        'type'     => 'text',
                        'label'    => $this->l('Flingex key'),
                        'required' => true,
                    ),
                    array(
                        'name'     => 'FLINGEX_USER_NAME',
                        'type'     => 'text',
                        'label'    => $this->l('User Name'),
                        'required' => true,
                    ),
                    array(
                        'name'     => 'FLINGEX_USER_PASSWORD',
                        'type'     => 'password',
    ...
    public function postInitDHLProcess()
    {
        if (Tools::isSubmit('submitSaveOptions')) {
            ...
            if (!in_array(Tools::getValue('DHLDP_DHL_API_VERSION'), FlingexDHLFlingexApi::$supported_shipper_countries[Tools::getValue('DHLDP_DHL_COUNTRY')]['api_versions'])) {
    ...
    public function postProcess()
    {
        ...
        */
        ...
        if (Tools::isSubmit('submitAddDHLDP_dhl_configure')) {
            ...
            if (count($form_errors) == 0) {
                if (in_array(Configuration::get('DHLDP_DHL_API_VERSION'), FlingexDHLFlingexApi::getSupportedApiVersions())) {
                    $this->flingex_api->setApiVersion(Configuration::get('DHLDP_DHL_API_VERSION'));
                }
                $check_client = $this->flingex_api->checkDHLAccount(
                    $dhl_mode,
                    ($dhl_mode == 1) ? FlingexDHLFlingexApi::$dhl_live_ciguser[$this->flingex_api->getApiVersion()] : FlingexDHLFlingexApi::$dhl_sbx_ciguser,
                    ($dhl_mode == 1) ? FlingexDHLFlingexApi::$dhl_live_cigpass[$this->flingex_api->getApiVersion()] : FlingexDHLFlingexApi::$dhl_sbx_cigpass,
                    ($dhl_mode == 1) ? $dhl_live_user : FlingexDHLFlingexApi::$dhl_sbx_user[$this->flingex_api->getApiVersion()],
                    ($dhl_mode == 1) ? $dhl_live_sign : FlingexDHLFlingexApi::$dhl_sbx_sign[$this->flingex_api->getApiVersion()]
                );
            ...
            if (count($form_errors) == 0) {
                $result_save = Configuration::updateValue('DHLDP_DHL_MODE', (int)Tools::getValue('DHLDP_DHL_MODE')) &&
                    ...
                    Configuration::updateValue('FLINGEX_URL', Tools::getValue('FLINGEX_URL')) &&
                    Configuration::updateValue('SANDBOX_URL', Tools::getValue('SANDBOX_URL')) &&
                    Configuration::updateValue('FLINGEX_USER_NAME', Tools::getValue('FLINGEX_USER_NAME')) &&
                    Configuration::updateValue('FLINGEX_USER_PASSWORD', Tools::getValue('FLINGEX_USER_PASSWORD'));
    ...
    public function getDPLabelData($id_order_carrier)
    {
        ...
        if (count($id_order_carrier) > 0) {
            $selected_values = Db::getInstance()->executeS(
                'SELECT * FROM `'._DB_PREFIX_.'flingex_dp_label` l
                 WHERE l.`id_order_carrier` IN ('.implode(',', array_map('intval', $id_order_carrier)).')'.
                ' ORDER BY `date_add`'
            );
    ...
    public function updateDPOrderCarrierWithTrackingNumber($id_order_carrier, $tracking_number)
    {
        ...
        if (Validate::isLoadedObject($order_carrier)) {
            ...
            if ($order_carrier->update()) {
                ...
                $tracking_url = str_replace('[tracking_number]', $tracking_number, FlingexApi::$tracking_url);
...
//       die('here');
    ...
    public static function callFlingexAPI($method, $url, $data = false,$headers = false)
    {
        $curl = curl_init();
        switch ($method)
        {
