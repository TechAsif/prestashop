...
class FLINGEXLabel extends ObjectModel
{
    ...
    {
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT * FROM `'._DB_PREFIX_.'flingex_package` dp
        WHERE dp.`id_flingex_label` = '.(int)$id_dhldp_label);
    }

    public static function getLabelIDByShipmentNumber($shipment_number)
    {
        return Db::getInstance()->getValue('SELECT id_flingex_label FROM `'._DB_PREFIX_.'flingex_label` WHERE shipment_number=\''.pSql($shipment_number).'\'');
    }
    ...
    public function deletePackages()
    {
        return Db::getInstance()->execute(
            'DELETE FROM `'._DB_PREFIX_.'flingex_package`
			WHERE `id_flingex_label` = '.(int)$this->id
        );
    }
}
