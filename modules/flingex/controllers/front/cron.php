<?php
// url: http://localhost/ps174/index.php?fc=module&module=flingex&controller=cron
// require_once dirname(__FILE__) . '/../../index.php';
class  flingexcronModuleFrontController extends ModuleFrontControllerCore
{
...
//                $this->indexProducts($action,  $total_indexed, $time);
    ...
    private function indexOrders($action, $total_indexed, $time)
    {
        ...
        foreach ($order_query as $key=>$value){
            ...
            $_the_sql = 'DELETE FROM `'._DB_PREFIX_.'flingex_order_tracking` WHERE `reference` = "'.$thi_ref.'"';

            Db::getInstance()->execute($_the_sql);

            $_SQL = 'SELECT po.reference, po.id_flingex_order, po.tracking_number FROM '._DB_PREFIX_.'flingex_order po where `reference` = "'.$thi_ref.'"';

            $query = Db::getInstance()->executeS($_SQL)[0];

            $tracking_api_response = self::sentToFlingexOrderTrackingApi($thi_ref);
            ...
            (`reference`, `id_flingex_order`, `tracking_number`,`tracking_event_key`,`tracking_event_value`,
            `api_response_status_code`,`api_response_status_message`)
            values(
             "' . $thi_ref . '",
             ' . $query['id_flingex_order'] . ',
...
//        Tools::redirect($url);
    ...
    public function sentToFlingexOrderTrackingApi($ref_number){
    ...
    public function callFlingexAPI($method, $url, $data = false,$headers = false)
    {
        $curl = curl_init();
        switch ($method)
        {
