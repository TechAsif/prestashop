    ...
    public function init()
    {
        ...
        if (Tools::getIsset('ajax')) {
            if (Tools::getIsset('getAddressAdditions')) {
                ...
                die(FlingexOrder::updatePermission((int)Context::getContext()->cart->id, (Tools::getValue('permission', 0)) == '1' ? 1 : 0));
            } elseif (Tools::getValue('action') == 'getprivate') {
                if (FlingexOrder::hasPermissionForTransferring((int)Context::getContext()->cart->id) === false) {
                    FlingexOrder::updatePermission((int)Context::getContext()->cart->id, 0);
                }
                $permission = FlingexOrder::hasPermissionForTransferring((int)Context::getContext()->cart->id);
