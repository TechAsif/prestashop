...
class ECourier extends Module
{
	...
	protected function displayECourierSettings()
	{
		...
		$fields_value_keys = array('USER_ID', 'API_KEY', 'API_SECRET', 'SANDBOX');
		$this->setFormFieldsValue($helper, $fields_value_keys);

		$form_fields = [
			'form'  => [
				'form' => [
					'id_form'     => 'ecourier_auth_settings',
					'legend'      => array(
						'title' => $this->l('ECourier Authentication settings'),
						'icon'  => 'icon-circle',
					),
					...
					'input'       => array(
						...
						array(
							'name'             => self::$conf_prefix . 'USER_ID',
							'type'             => 'text',
							'label'            => $this->l('USER-ID'),
							'required'         => true,
						),
						array(
							'name'             => self::$conf_prefix . 'API_KEY',
							'type'             => 'text',
							'label'            => $this->l('API-KEY'),
							'required'         => true,
						),
						array(
							'name'             => self::$conf_prefix . 'API_SECRET',
							'type'             => 'text',
	...
	public function postProcess()
	{
		...
		if (Tools::isSubmit('submitAddconfiguration')) {
			...
			$USER_ID = Tools::getValue(self::$conf_prefix . 'USER_ID');
			$API_KEY = Tools::getValue(self::$conf_prefix . 'API_KEY');
			$API_SECRET = Tools::getValue(self::$conf_prefix . 'API_SECRET');
			$SANDBOX = Tools::getValue(self::$conf_prefix . 'SANDBOX');

			if ($USER_ID == '') {
				$form_errors[] = $this->_errors[] = $this->l('Please fill USER ID');
			}

			if ($API_KEY == '') {
				$form_errors[] = $this->_errors[] = $this->l('Please fill API KEY');
			}
			if ($API_SECRET == '') {
				$form_errors[] = $this->_errors[] = $this->l('Please fill API SECRET');
			}
			if ($SANDBOX == '') {
				$form_errors[] = $this->_errors[] = $this->l('Please fill API SECRET');
			}
			...
			if (count($form_errors) == 0) {
				$result_save = 
					Configuration::updateValue(self::$conf_prefix . 'USER_ID', Tools::getValue(self::$conf_prefix . 'USER_ID')) &&
					Configuration::updateValue(self::$conf_prefix . 'API_KEY', Tools::getValue(self::$conf_prefix . 'API_KEY')) &&
					Configuration::updateValue(self::$conf_prefix . 'API_SECRET', Tools::getValue(self::$conf_prefix . 'API_SECRET'));
